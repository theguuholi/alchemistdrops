...
- Define one documented type alias for every schema field represented in `t()`. Each `@typedoc` explains the field's domain meaning and relevant nullability or lifecycle state, while its `@type` holds the complete value type. Reference only those aliases from `t()`; do not inline field types there (for example, use `stripe_product_id: stripe_product_id()`, not `stripe_product_id: String.t() | nil`).
